...
use commands::{claude, files, fileserver, git, jupyter, pty, spotify, updater};
...
impl FocusState {
    fn new(initial: &str) -> Self {
        Self(Mutex::new(initial.to_string()))
    }
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let open_file =
                MenuItem::with_id(h, "open_file", "Open File…", true, Some("CmdOrCtrl+O"))?;
            ...
            let close_tab =
                MenuItem::with_id(h, "close_tab", "Close Tab", true, Some("CmdOrCtrl+W"))?;
            ...
            // ── Selection ────────────────────────────────────────────────────
            let sel_line =
                MenuItem::with_id(h, "sel_line", "Select Line", true, Some("CmdOrCtrl+L"))?;
            let sel_all = MenuItem::with_id(h, "sel_all", "Select All", true, Some("CmdOrCtrl+A"))?;

            let sel_menu = Submenu::with_items(h, "Selection", true, &[&sel_all, &sel_line])?;
            ...
            let toggle_git = MenuItem::with_id(
                h,
                "toggle_git",
                "Toggle Git Panel",
                true,
            ...
            let toggle_settings =
                MenuItem::with_id(h, "toggle_settings", "Settings", true, Some("CmdOrCtrl+,"))?;
            ...
            // ── Go ───────────────────────────────────────────────────────────
            let go_file =
                MenuItem::with_id(h, "go_file", "Go to File…", true, Some("CmdOrCtrl+P"))?;
            let go_palette = MenuItem::with_id(
                h,
                "go_palette",
                "Command Palette…",
                true,
            ...
            let go_menu = Submenu::with_items(h, "Go", true, &[&go_file, &go_palette])?;
            ...
            let help_menu = Submenu::with_items(h, "Help", true, &[&help_shortcuts])?;

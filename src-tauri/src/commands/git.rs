...
pub async fn git_status(repo_path: String) -> Result<Vec<GitFileStatus>, String> {
    ...
    let statuses = StatusManager::new(&repo)
        .list()
...
pub async fn git_discard(repo_path: String, file_path: String) -> Result<(), String> {
    git_run_async(vec!["checkout".into(), "--".into(), file_path], repo_path)
        .await
        .map(|_| ())
}
...
pub async fn git_stash_list(repo_path: String) -> Result<Vec<GitStash>, String> {
    ...
    let stashes = raw
        ...
        .filter_map(|(i, line)| {
            ...
            let branch = if let Some(rest) = msg
                .strip_prefix("On ")
...
pub async fn git_commit_amend(repo_path: String, message: String) -> Result<String, String> {
    ...
    .await
    .map(|out| out.trim().lines().next().unwrap_or("").to_string())
}
...
pub async fn git_ahead_behind(repo_path: String, branch: String) -> Result<AheadBehind, String> {
    ...
    match git_run(
        &["rev-list", "--left-right", "--count", &refspec],
        &repo_path,
    ) {
...
fn assign_lanes(commits: &mut [GraphCommit]) {
    ...
    for i in 0..commits.len() {
        ...
        let tracking: Vec<usize> = lanes
            .iter()
            .enumerate()
            .filter_map(|(j, s)| {
                if s.as_deref() == Some(&full_oid) {
                    Some(j)
                } else {
                    None
                }
        ...
        for parent in parents.iter().skip(1) {
            ...
            if lanes.iter().any(|s| s.as_deref() == Some(parent)) {
                continue;
            } // already tracked
...
fn strip_ref_prefix(r: &str) -> &str {
    ...
    if let Some(s) = r.strip_prefix("refs/tags/") {
        return s;
    }
    r
}
